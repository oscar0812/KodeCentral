 ...
 * @method     ChildCommentQuery orderByPostedTime($order = Criteria::ASC) Order by the posted_time column
 ...
 * @method     ChildCommentQuery groupByPostedTime() Group by the posted_time column
 ...
 *
 * @method     \UserQuery|\PostQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildComment findOneByPostedTime(string $posted_time) Return the first ChildComment filtered by the posted_time column
 * @method     ChildComment findOneByUserId(int $user_id) Return the first ChildComment filtered by the user_id column
 * @method     ChildComment findOneByPostId(int $post_id) Return the first ChildComment filtered by the post_id column *
 ...
 * @method     ChildComment requireOneByPostedTime(string $posted_time) Return the first ChildComment filtered by the posted_time column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildComment[]|ObjectCollection findByPostedTime(string $posted_time) Return ChildComment objects filtered by the posted_time column
...
abstract class CommentQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, text, posted_time, user_id, post_id FROM comment WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query on the posted_time column
     *
     * Example usage:
     * <code>
     * $query->filterByPostedTime('2011-03-14'); // WHERE posted_time = '2011-03-14'
     * $query->filterByPostedTime('now'); // WHERE posted_time = '2011-03-14'
     * $query->filterByPostedTime(array('max' => 'yesterday')); // WHERE posted_time > '2011-03-13'
     * </code>
     *
     * @param     mixed $postedTime The value to use as filter.
     ...
     * @return $this|ChildCommentQuery The current query, for fluid interface
     */
    public function filterByPostedTime($postedTime = null, $comparison = null)
    {
        if (is_array($postedTime)) {
            $useMinMax = false;
            if (isset($postedTime['min'])) {
                $this->addUsingAlias(CommentTableMap::COL_POSTED_TIME, $postedTime['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($postedTime['max'])) {
                $this->addUsingAlias(CommentTableMap::COL_POSTED_TIME, $postedTime['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(CommentTableMap::COL_POSTED_TIME, $postedTime, $comparison);
    }
